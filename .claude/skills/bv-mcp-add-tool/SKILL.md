...
17. **`server.json` description** — `server-json-tool-count.audit.test.ts` parses "N MCP tools"; bump it.
18. **README + docs + VS Code** — `readme-tool-surface.audit.test.ts` derives the count from `TOOLS.length` and asserts the prose matches: README (badge, prose, `N MCP tools`), `docs/github-settings.md`, `extensions/vscode/README.md` (`N DNS & email security tools`, `All N tools`, `## Tools (N)`, and the `M check_* tools` count if your tool is `check_*`), and `extensions/vscode/package.json`. Update those prose strings to the new count — the audit no longer carries a hardcoded number to bump.
