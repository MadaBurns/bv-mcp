...
The full binding inventory — name, type, purpose — plus the per-binding narratives
(activation, failure semantics, incident history) live in the **`bv-mcp-operations` skill**
("Binding reference table" and "Binding notes"). Load it before adding, removing, or
debugging a binding.
