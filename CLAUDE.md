...
Exposes 31 tools via MCP Streamable HTTP (JSON-RPC 2.0) at `https://dns-mcp.blackveilsecurity.com/mcp`.
...
| `BV_DOH_ENDPOINT` | var | Custom secondary DoH resolver URL (e.g. `https://internal-doh.example.com/dns-query`); optional — falls back to Google DoH when absent |
