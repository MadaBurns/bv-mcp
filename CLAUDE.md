...
- **SPF trust-surface — scored ONCE (1.8.0, #637)**: the **per-platform** "delegates to shared platform: X" findings are **always `info`** (penalty 0, but present and fully detailed). The single scored signal is the **aggregate** "SPF trust surface: N shared platforms" finding (emitted at 2+ platforms), elevated to `high` only when weak DMARC/relaxed alignment corroborates. Before 1.8.0 both halves were charged — −15 per platform PLUS the aggregate's −25 — so github.com's valid, working SPF record clamped to **0, identical to publishing no SPF at all**; SPF is one of only four categories the 1000-domain corpus found to discriminate, so that had to go. Do not reintroduce a per-platform penalty. ⚠️ The ESP catalog + analyzer are **duplicated** — core `packages/dns-checks/src/checks/spf-trust-surface.ts` + worker copy `src/tools/spf-trust-surface.ts`; `augmentTrustSurface()` (`check-spf.ts`) **replaces** the core's trust-surface findings (never the score) and rebuilds its DMARC context from the CORE finding's metadata. Edit BOTH copies identically; parity is audited by `test/audits/spf-trust-surface-catalog-parity.audit.test.ts`. Adding a platform to the CORE catalog is still a scoring change (it moves `platformCount`, and 2+ is what emits the scored aggregate at all) owing a `SCORING_MODEL_VERSION` bump (Mailjet, 1.7.0, #572). A worker-only entry leaves direct package consumers (bv-web-prod) under-counting. Depth: `bv-mcp-scoring` skill.
...
- **`passed`**: `score >= 50 && !hasMissingControl`. Missing control → score zeroed. Checks whose **absence** emits `missingControl: true`: HTTP Security, MX, NS, Zone Hygiene, BIMI, DANE — that is the whole list. CAA, **MTA-STS**, SVCB-HTTPS, TLS-RPT deliberately do NOT (absence is a graded finding; MTA-STS joined that list in scoring model 1.6.0 after a 1,000-domain corpus measured it at mean 3.3 with 96.5% of measured domains at 0 — a flat penalty, not a discriminator). **DNSSEC is the nuanced one**: absence is a `high` Core penalty via a fixed `penaltyOverride: 40` (category → 60, decoupling the −40 from the `high` label), NOT zeroing — but a **broken** chain (`chain of trust incomplete`, `validation failing`) DOES set the flag. Verify at the call sites before trusting any list: `grep -rn 'missingControl: true'` also matches the COMMENTS explaining why a check declines to set it. Rationale + evidence: `bv-mcp-scoring` skill.
- **Grades — TWO scales, by role (v3.26.0+, #461).** The **canonical 9-band** `scoreToGrade` (A+ 92+, A 87–91, B+ 82–86, B 76–81, C+ 70–75, C 63–69, D+ 56–62, D 50–55, F <50) is the internal/SSOT scale — used by `compare_baseline` ordering, the `/badge` SVG, `analyze_drift`, `map_compliance`, `generate_fix_plan`, the cohort-percentile math, golden tests, and `ScanScore.grade`. The **customer-facing display scale is the NIST-aligned 6-band** `nistScoreToGrade` (A+≥95, A≥90, B≥80, C≥70, D≥60, F<60), shown by `scan_domain`/`batch_scan`/`compare_domains` ONLY (recomputed from the 0–100 score at the `format-report.ts` `displayGradeFor` chokepoint; `'N/A'` preserved). Both live in `@blackveil/dns-checks` (the 6-band is additive, 1.4.0+); bv-web-prod displays the SAME NIST letter everywhere so a domain shows one grade across web + MCP. `PARITY_CORPUS_VERSION` must move in lockstep with the package version (version-lock contract). Scores are unchanged by the display scale — only the letter.
