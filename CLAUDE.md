...
- **`passed`**: `score >= 50 && !hasMissingControl`. Missing control → score zeroed. Checks using `missingControl: true`: HTTP Security, MX, NS, Zone Hygiene, BIMI, DANE. (CAA, **MTA-STS**, SVCB-HTTPS, TLS-RPT deliberately do NOT — absence is a graded finding, not a category-zeroing missing control.) **MTA-STS moved to the deliberately-NOT list in scoring model 1.6.0**: a 1,000-domain corpus (2026-08-03) measured `mta_sts` at mean 3.3 with 96.5% of 687 measured domains at exactly 0, so zeroing on absence was a flat constant penalty on ~3 of ~80 base points rather than a discriminator. Weight (3, protective) and severities unchanged; a deployed-but-BROKEN policy still emits its confident `high`/`medium` findings (which never carried `missingControl`). Rationale block: `MTA_STS_ABSENCE_IS_GRADED_NOT_ZEROING` in `packages/dns-checks/src/checks/mta-sts-analysis.ts`. **DNSSEC deliberately does NOT** — per NIST SP 800-81r3, DNSSEC is a baseline integrity control in a defense-in-depth model: absence is a `high`-severity Core penalty (category → 60, via a fixed `penaltyOverride: 40` on the finding metadata that decouples the −40 deduction from the `high` severity label — see `computeCategoryScore`), not a category-zeroing missing control.
