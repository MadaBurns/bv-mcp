...
| `BV_TLS_PROBE` / `BV_TLS_PROBE_KEY`                                                | Service / Secret | **Operator only.** Legacy-TLS detection for `check_ssl`; fail-soft. ⚠️ TLS-intercepted vantage — its `certificate` is never the origin's; DANE pin verification kill-switched (3.75.1, #906). Depth: `bv-mcp-operations` → Binding notes |
