...
| `BV_TLS_PROBE` / `BV_TLS_PROBE_KEY`                                                | Service / Secret | **Operator only.** Legacy-TLS detection for `check_ssl`; fail-soft. ⚠️ Its `certificate` capture is NOT the origin's — Browser Rendering re-signs TLS (Mockttp), so DANE pin verification is kill-switched (`DANE_PIN_VERIFICATION_ENABLED`, 3.75.1); its `minVersion` legacy-TLS signal likely describes the proxy handshake too (bv-web-prod#2848) |
