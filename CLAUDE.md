...
Both publish together via the manual `npm publish` path (npm publish is currently gated off — #719); `publish.yml` no longer carries publish jobs.
...
| `ANALYTICS_PII_LEVEL` | var | Access-log PII depth: `coarse` (default) \ |
...
| `BV_DOH_ENDPOINT` / `BV_DOH_TOKEN` | Secret / Secret | Optional private secondary DoH, consulted only when the primary returns no typed answers. ⚠️ Both are Secrets, never `vars`. [Notes](#binding-notes) |
| `CERTSPOTTER_TOKEN` | Secret | SSLMate Cert Spotter API key for the CT source. Optional and fail-soft; raises rate limits ONLY. [Notes](#binding-notes) |
...
| `BV_RECON` | Service | **Operator-deploy only.** bv-recon OSINT/recon worker behind the recon tools. Fail-soft → `unprovisioned`. [Notes](#binding-notes) |
| `BV_RECON_KEY` | Secret | Bearer token for bv-recon's admin routes (= bv-recon `ADMIN_API_KEY` / its dedicated `BV_MCP_KEY`). |
| `BV_TLS_PROBE` | Service | **Operator-deploy only.** Version-aware TLS handshakes so `check_ssl` can detect legacy TLS. Fail-soft. [Notes](#binding-notes) |
| `BV_TLS_PROBE_KEY` | Secret | Bearer token for bv-tls-probe. |
| `BV_WEB` | Service | **Operator-deploy only.** OAuth consent proxy + the M365 `m365Proxy`. ⚠️ Unlike the other private bindings it IS declared in public `wrangler.jsonc`. [Notes](#binding-notes) |
...
| `BV_INFRA_PROBE` | Service | Authoritative-DNS/BGP/RPKI vantage probe for `authoritative_dns_infra`. ⚠️ NOT overlay-only, and `npm run deploy:prod` does NOT deploy it. [Notes](#binding-notes) |
...
| `ALERT_WEBHOOK_URL` / `ALERT_*_THRESHOLD` / `ALERT_LOOKBACK_MINUTES` / `ALERT_LATENCY_LOOKBACK_MINUTES` / `ALERT_MIN_LATENCY_SAMPLES` | var | Cron alerts. ⚠️ Latency has its OWN lane and window; `ALERT_WEBHOOK_URL` is the STATIC fallback only. [Notes](#binding-notes) |

### Binding notes

Detail for the bindings whose Purpose cell above is a summary. Kept out of the table because a single long cell pads **every** row to its width — six narrative cells were inflating this file by ~47 KB (~12k tokens per session).
