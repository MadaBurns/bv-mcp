...
src/tools/scan/           — Scan sub-helpers (format-report.ts, post-processing.ts)
...
- `.github/workflows/release.yml`: typecheck + test + GitHub release on tag push (`v*`). Deployment to Cloudflare is done manually via `npm run deploy` from a local machine with `.dev/wrangler.local.jsonc` configured.
