...
- **SPF trust-surface**: shared-platform findings informational; elevated only when weak DMARC/relaxed alignment corroborates. ⚠️ The ESP catalog is **duplicated** — core `packages/dns-checks/src/checks/spf-trust-surface.ts` + worker copy `src/tools/spf-trust-surface.ts`; `augmentTrustSurface()` (`check-spf.ts`) **replaces** the core's trust-surface findings and rebuilds its DMARC context from the CORE finding's metadata. So adding a platform to the CORE catalog changes **severities**, not just counts (uncataloged → always the `info` fallback; cataloged → `medium` under `p=none` + relaxed alignment) — a scoring change owing a `SCORING_MODEL_VERSION` bump (Mailjet, 1.7.0, #572). A worker-only entry leaves direct package consumers (bv-web-prod) under-counting. Depth: `bv-mcp-scoring` skill.
