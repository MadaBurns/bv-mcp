...
| `BV_INFRA_PROBE`                                                     | Service          | **Operator-deploy only.** Raw authoritative-DNS/BGP/RPKI vantage-probe binding for the `authoritative_dns_infra` check (distinct from `BV_INFRA_GRAPH`). Fail-soft when absent. |
