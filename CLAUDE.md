...
**Branch protection** (verified 2026-06-05): required checks = `build-and-test`, `Secret & PII scan`, `Dependency audit`, `File hygiene check` (`strict=true` — branch must be up to date). **No approving review is required** (`required_pull_request_reviews=null`); `required_conversation_resolution=true` still applies. **`enforce_admins=false`** — a green PR is CLEAN to merge once required checks pass; admins may also bypass via `--admin`. `allow_force_pushes=false`; use the PR path rather than pushing to `main` directly. **`dns-scan` is NOT required** and stays pending for hours; `mergeStateStatus: UNSTABLE` is mergeable once the required checks are satisfied.
