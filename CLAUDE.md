...
⚠️ **Adding a workflow job that pins `actions/setup-node` trips gitleaks' `phone-number` rule — this one IS a `--no-verify` case.** A long all-digit run inside the 40-hex SHA scans as a phone number, so gitleaks blocks the commit on a line that is a **mandatory** SHA pin. (Do NOT quote the offending digits here to illustrate the point — doing so re-trips the rule on this very file, which is exactly what happened when this warning was first written, and mirrors the dotted-quad warning above whose own examples had to be despecified.) (the F11 audit in `test/audits/workflow-permissions.audit.test.ts` requires 40-hex pinning plus a `# vX.Y.Z` comment). Unlike the dotted-quad case above there is no rewording available — you cannot respell a commit SHA, and the identical pin already appears elsewhere in `ci.yml`, so gitleaks only flags it because it scans the **diff**, not the file. Verify it is that line and nothing else (`git diff --cached | grep setup-node`), confirm the SHA already exists on `main`, then hand-check the other four pre-commit gates — blocked paths, generated files, **staged symlinks** (`git diff --cached --raw | awk '$2 ~ /^120000$/'`), and `npm run audit:repo-safety` — because `--no-verify` skips all five, not just gitleaks. Still do NOT add a gitleaks allowlist entry: an allowlist broad enough to cover arbitrary hex would blind the rule generally.
