...
**Version**: 2.0.8 — keep `SERVER_VERSION` in `src/lib/server-version.ts` and `version` in `package.json` in sync.
...
**Consumers**: bv-web (`/admin/bulk-scanner` → batch, individual pages → call via `bvMcpClient`).
