...
def test_nosend_domain():
    ...
    # Scan brand-auditdbs.com (no-send domain: v=spf1 -all + MX)
    status1, _, body1 = tool_call("scan_domain", {"domain": "brand-auditdbs.com", "format": "compact"}, sid, msg_id=10)
    if status1 == 200 and isinstance(body1, dict) and "result" in body1:
        ...
        if "DKIM" in text:
            ...
            if dkim_match:
                ...
                if dkim_score == 100:
                    log("PASS", f"brand-auditdbs.com DKIM score={dkim_score} (no-send downgrade works)")
                else:
                    log("FAIL", f"brand-auditdbs.com DKIM score={dkim_score}, expected 100")
            else:
                # Maybe the text format doesn't include numeric scores; check for PASS
                if "DKIM" in text and ("100" in text or "pass" in text.lower()):
                    log("PASS", "brand-auditdbs.com DKIM appears passing (no-send downgrade)")
                else:
                    log("FAIL", "Could not parse DKIM score from brand-auditdbs.com scan", text[:300])
        else:
            log("FAIL", "DKIM not found in brand-auditdbs.com scan result", text[:300])
    else:
        log("FAIL", "brand-auditdbs.com scan failed", str(body1)[:200])
    ...
    status3, _, body3 = tool_call("check_spf", {"domain": "brand-auditdbs.com", "format": "compact"}, sid, msg_id=12)
    if status3 == 200 and isinstance(body3, dict) and "result" in body3:
        ...
        if content3 and any(b.get("text", "") for b in content3):
            log("PASS", "check_spf on brand-auditdbs.com returned data")
        else:
            log("FAIL", "check_spf on brand-auditdbs.com returned empty content")
    else:
        log("FAIL", "check_spf on brand-auditdbs.com failed", str(body3)[:200])
...
def test_concurrent_stress():
    print("\n=== TEST 6: Concurrent Stress — 20 Parallel Scans ===")
    domains = [
        "google.com", "cloudflare.com", "[redacted-domain]", "github.com", "microsoft.com",
        "apple.com", "amazon.com", "netflix.com", "openai.com", "linear.app",
        "tesla.com", "brand-lambda.com", "spotify.com", "slack.com", "discord.com",
        "twitch.tv", "reddit.com", "[redacted-domain]", "shopify.com", "zoom.us"
    ]
