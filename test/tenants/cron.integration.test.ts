...
const cscWeeklyRescanMock = vi.hoisted(() => vi.fn(async (_e: unknown, _c: unknown) => undefined));
const cscCycleAlertsMock = vi.hoisted(() => vi.fn(async (_e: unknown, _c: unknown) => undefined));
...
vi.mock('../../src/tenants/scheduled-handlers', async () => {
	...
	return {
		...actual,
		handleTenantWeeklyRescan: cscWeeklyRescanMock,
		handleTenantCycleAlerts: cscCycleAlertsMock,
	};
});
...
beforeEach(() => {
	cscWeeklyRescanMock.mockClear();
	cscCycleAlertsMock.mockClear();
...
describe('worker.scheduled cron dispatch', () => {
	it('a. weekly cron 0 2 * * 0 → handleTenantWeeklyRescan runs (and only it)', async () => {
		await runCron('0 2 * * 0');
		expect(cscWeeklyRescanMock).toHaveBeenCalledTimes(1);
		expect(cscCycleAlertsMock).not.toHaveBeenCalled();
	...
	it('b. 15-min cron *\\/15 * * * * → handleFuzzingScan AND handleTenantCycleAlerts both run', async () => {
		...
		expect(cscCycleAlertsMock).toHaveBeenCalledTimes(1);
		expect(scheduledMock).toHaveBeenCalledTimes(1);
		expect(cscWeeklyRescanMock).not.toHaveBeenCalled();
		expect(dailyDigestMock).not.toHaveBeenCalled();
	});
	...
	it('d. rejection in handleTenantCycleAlerts does NOT prevent the other two handlers from being scheduled', async () => {
		...
		cscCycleAlertsMock.mockImplementationOnce(async () => {
