...
import { tenantKV } from '../../src/tenants/adapters/tenant-kv';
...
describe('tenantKV adapter', () => {
	it('stamps tenant prefix on put/get/delete and composes list prefix', async () => {
		...
		const adapter = tenantKV(binding, 'tenant');
		...
		expect(calls).toEqual([
			{ method: 'put', key: 'tenant:cache:example.com', opts: { expirationTtl: 60 } },
			{ method: 'get', key: 'tenant:cache:example.com' },
			{ method: 'delete', key: 'tenant:cache:example.com' },
			{ method: 'list', opts: { prefix: 'tenant:cache:' } },
			{ method: 'list', opts: { prefix: 'tenant:' } },
		]);
	});
	...
	it('rejects empty / unsafe prefix at construction (cross-tenant leak guard)', () => {
		...
		expect(() => tenantKV(binding, 'tenant:sub')).toThrow(/prefix/i);
		expect(() => tenantKV(binding, 'tenant/foo')).toThrow(/prefix/i);
	});
	...
	it('isolates two adapters with different prefixes', async () => {
		...
		await tenantKV(b1, 'tenant').put('quota', '1');
		await tenantKV(b2, 'acme').put('quota', '1');

		expect(c1[0]).toMatchObject({ method: 'put', key: 'tenant:quota' });
	...
	it('forwards binding errors instead of swallowing them', async () => {
		...
		const adapter = tenantKV(failing, 'tenant');
