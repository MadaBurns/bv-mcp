...
import { tenantR2 } from '../../src/tenants/adapters/tenant-r2';
...
describe('tenantR2 adapter', () => {
	it('stamps tenant prefix on put/get/delete and composes list prefix', async () => {
		...
		const adapter = tenantR2(binding, 'tenant');
		...
		expect(calls).toEqual([
			{ method: 'put', key: 'tenant/reports/2026-05.json' },
			{ method: 'get', key: 'tenant/reports/2026-05.json' },
			{ method: 'delete', key: 'tenant/reports/2026-05.json' },
			{ method: 'list', opts: { prefix: 'tenant/reports/' } },
	...
	it('rejects empty / unsafe prefix at construction (cross-tenant leak guard)', () => {
		...
		expect(() => tenantR2(binding, 'tenant/sub')).toThrow(/prefix/i);
	});
	...
	it('isolates two adapters with different prefixes', async () => {
		...
		await tenantR2(b1, 'tenant').put('report.json', new ReadableStream());
		await tenantR2(b2, 'acme').put('report.json', new ReadableStream());

		expect(c1).toEqual([{ method: 'put', key: 'tenant/report.json' }]);
	...
	it('forwards binding errors instead of swallowing them', async () => {
		...
		const adapter = tenantR2(failing, 'tenant');
