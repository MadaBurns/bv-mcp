...
describe('parseHackerOneScope', () => {
	it('extracts URL assets as concrete domains', () => {
		const out = parseHackerOneScope({
			structured_scopes: [
				{ asset_identifier: 'marriott.com', asset_type: 'URL', eligible_for_submission: true },
			],
		});
		expect(out).toEqual([
	...
	it('marks wildcard scopes', () => {
		const out = parseHackerOneScope({
			structured_scopes: [
				{ asset_identifier: '*.marriott.com', asset_type: 'WILDCARD', eligible_for_submission: true },
			],
		});
		expect(out[0]).toMatchObject({ isWildcard: true, domain: 'marriott.com', assetType: 'wildcard' });
	});
...
describe('parseBugcrowdScope', () => {
	it('extracts website targets', () => {
		const out = parseBugcrowdScope({
			targets: [{ name: 'marriott.com', category: 'website', in_scope: true }],
		});
		expect(out[0]).toMatchObject({ domain: 'marriott.com', assetType: 'url', inScope: true, platform: 'bugcrowd' });
	});
...
describe('parseIntigritiScope', () => {
	it('extracts endpoint domains', () => {
		const out = parseIntigritiScope({
			domains: [{ endpoint: 'https://marriott.com', type: 'url' }],
		});
		expect(out[0]).toMatchObject({ domain: 'marriott.com', assetType: 'url', platform: 'intigriti' });
	});

	it('extracts wildcard endpoints', () => {
		const out = parseIntigritiScope({
			domains: [{ endpoint: '*.marriott.com', type: 'url' }],
		});
		expect(out[0]).toMatchObject({ isWildcard: true, domain: 'marriott.com' });
	});
...
describe('detectBountyScope (orchestrator)', () => {
	...
	it('returns ok + canonical schema-conforming result when all platforms succeed', async () => {
		...
		const result = await detectBountyScope('marriott.com', {
		...
		expect(result.wildcardScopes).toEqual(['marriott.com']);
	...
	it('returns partial when one platform fails and another succeeds', async () => {
		const fetcher = fetcherFromMap({
			'https://hackerone.com/marriott.json': {
				status: 200,
				body: { structured_scopes: [{ asset_identifier: 'marriott.com', asset_type: 'URL', eligible_for_submission: true }] },
			},
	...
	it('returns failed when no handles are supplied (nothing to query)', async () => {
		...
		const result = await detectBountyScope('marriott.com', { handles: {}, fetcher });
		expect(result.queryStatus).toBe('failed');
	...
	it('dedupes a domain that appears across multiple platforms', async () => {
		const fetcher = fetcherFromMap({
			...
			'https://bugcrowd.com/marriott.json': {
				status: 200,
				body: { targets: [{ name: 'marriott.com', category: 'website', in_scope: true }] },
			},
		});
		const result = await detectBountyScope('marriott.com', {
			handles: { hackerone: 'marriott', bugcrowd: 'marriott' },
