// Tenant capacity-readiness invariants. This is the audit-layer codification
// of the math in tenant-Scalable-Architecture-Design.md §1.2 — the configuration
// values that make 2.5M-domain portfolio audits possible.
//
// If any of these drift toward Tenant-incompatible values, this test fails in
// CI before a regression reaches prod. Update both this test AND the design
// doc together if a Tenant-class threshold legitimately changes.
...
/** Headline target. */
const Tenant_HEADLINE_PORTFOLIO = 2_500_000;

describe('Tenant capacity-readiness invariants', () => {
	describe('Quota headroom (Phase 0)', () => {
		it('partner.scan_domain quota covers a one-shot 2.5M audit', () => {
			expect(TIER_TOOL_DAILY_LIMITS.partner?.scan_domain).toBeGreaterThanOrEqual(Tenant_HEADLINE_PORTFOLIO);
		});
		...
		it('GLOBAL_DAILY_TOOL_LIMIT does not constrain authenticated Tenant traffic', () => {
			// Sanity check: it's a positive number, not infinity, but irrelevant to
			// the Tenant code path because authenticated callers / internal binding
	...
	describe('Timeout budget (per-scan latency envelope)', () => {
		it('SCAN_TIMEOUT_MS leaves headroom over observed p50', () => {
			...
			// p50, ~half of Tenant scans would return partial results.
	...
	describe('Throughput projection — completes within 24h SLO', () => {
		it('projects <24h for the headline 2.5M-domain audit', () => {
			...
			const projectedRuntimeSeconds = Tenant_HEADLINE_PORTFOLIO / throughputDomainsPerSecond;
