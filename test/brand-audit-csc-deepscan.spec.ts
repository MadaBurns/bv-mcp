...
describe('runDeepScan', () => {
	it('runs scan_domain + discover_subdomains for each apex (parallel cap 5)', async () => {
		...
		expect(result.deepScan.subdomainInventoryByApex['[redacted-domain]'].total).toBe(42);
		expect(result.deepScan.subdomainInventoryByApex['[redacted-domain]'].source).toBe('certificate_transparency');
	});
