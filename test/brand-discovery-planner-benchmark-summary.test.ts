...
describe('summarizeBenchmark', () => {
	it('pairs observe/enforce rows by domain and computes reduction and surfaced delta', () => {
		const summary = summarizeBenchmark({
			rows: [
				row({ domain: 'walmart.com', mode: 'observe' }),
				row({
					domain: 'walmart.com',
					mode: 'enforce',
					elapsedMs: 40000,
		...
		expect(pair.domain).toBe('walmart.com');
	...
	it('aggregates pass counts across multiple domain pairs', () => {
		const summary = summarizeBenchmark({
			rows: [
				row({ domain: 'walmart.com', mode: 'observe' }),
				row({ domain: 'walmart.com', mode: 'enforce', metrics: { candidateSignalProbes: 580, plannerEfficiency: { mode: 'enforce', candidateSignalProbes: 580, baselineCandidateSignalProbes: 1050, surfacedCandidates: 2 } } }),
				row({ domain: 'bankofamerica.com', mode: 'observe', metrics: { plannerEfficiency: { mode: 'observe', candidateSignalProbes: 1050, baselineCandidateSignalProbes: 1050, surfacedCandidates: 3 } } }),
				row({ domain: 'bankofamerica.com', mode: 'enforce', metrics: { candidateSignalProbes: 580, plannerEfficiency: { mode: 'enforce', candidateSignalProbes: 580, baselineCandidateSignalProbes: 1050, surfacedCandidates: 3 } } }),
				row({ domain: 'marriott.com', mode: 'observe', metrics: { plannerEfficiency: { mode: 'observe', candidateSignalProbes: 1050, baselineCandidateSignalProbes: 1050, surfacedCandidates: 8 } } }),
				row({ domain: 'marriott.com', mode: 'enforce', metrics: { candidateSignalProbes: 700, plannerEfficiency: { mode: 'enforce', candidateSignalProbes: 700, baselineCandidateSignalProbes: 1050, surfacedCandidates: 8 } } }),
			],
		});
...
describe('formatAcceptanceSummary', () => {
	it('renders a per-domain table and overall acceptance verdict', () => {
		...
		expect(output).toContain('walmart.com');
