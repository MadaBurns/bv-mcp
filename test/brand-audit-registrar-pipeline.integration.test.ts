 ...
 *   - [redacted-domain].au: signals ['dkim_key_reuse', 'ns'], registrar on the corporate-domains family (same family as target).
...
describe('runBrandAuditPipeline with view=registrar_complement', () => {
	...
	it('emits registrarComplement on the result when view=registrar_complement', async () => {
		...
		// [redacted-domain].au → will classify as consolidated (same corporate-domains registrar family, strong dkim signal)
		...
		// RDAP stubs: target [redacted-domain] → corporate-domains registrar; ford.co.uk → GoDaddy; [redacted-domain].au → corporate-domains registrar.
	...
	it('persists registrar_complement_fast step when stepStore is provided', async () => {
		...
		const record = await stepStore.get(auditId, '[redacted-domain]', 'registrar_complement_fast');
	...
	it('runs runDeepScanFromStepStore and merges to registrar_complement_full', async () => {
		...
		stored.set('a-1:[redacted-domain]:registrar_complement_fast', {
			status: 'completed',
			payload: {
				viewVersion: 2,
		...
		const full = stored.get('a-1:[redacted-domain]:registrar_complement_full') as {
			status: string;
