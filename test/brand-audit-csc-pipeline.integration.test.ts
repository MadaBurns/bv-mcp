 ...
 *     Rule 2 → isExactBrandPortfolioDomain(ford.co.uk, [redacted-domain])=true → realShadowItClassification
 *     → shadowIt/owned_off_primary_registrar.
 *   - [redacted-domain].au: signals ['dkim_key_reuse', 'ns'], registrar CSC (same family as target).
 *     Rule 2 → isExactBrandPortfolioDomain([redacted-domain].au, [redacted-domain])=true → isOffPrimaryRegistrar=false
...
describe('runBrandAuditPipeline with view=csc_complement', () => {
	...
	it('emits cscComplement on the result when view=csc_complement', async () => {
		...
		setupEnrichmentFetchMock(['ford.co.uk', '[redacted-domain].au']);
		...
		// [redacted-domain].au → will classify as consolidated (CSC, same registrar family, strong dkim signal)
		const discoverBrandDomains = async () =>
			makeDiscoveryResult('[redacted-domain]', [
				{ domain: 'ford.co.uk', signals: ['dkim_key_reuse', 'san'] },
				{ domain: '[redacted-domain].au', signals: ['dkim_key_reuse', 'ns'] },
			]);

		// RDAP stubs: target [redacted-domain] → CSC; ford.co.uk → GoDaddy; [redacted-domain].au → CSC.
		...
		expect(parsed.anchor.apex).toBe('[redacted-domain]');
	...
	it('persists csc_complement_fast step when stepStore is provided', async () => {
		...
		const discoverBrandDomains = async () =>
			makeDiscoveryResult('[redacted-domain]', [{ domain: 'ford.co.uk', signals: ['dkim_key_reuse', 'san'] }]);
		const checkRdapLookup = async (domain: string) => {
			if (domain === 'ford.co.uk') return makeRdapResult('GoDaddy.com, LLC');
			return makeRdapResult('CSC Corporate Domains, Inc.');
		};
		...
		const record = await stepStore.get(auditId, '[redacted-domain]', 'csc_complement_fast');
	...
	it('enqueues a deep_scan message after fast_ready', async () => {
		...
		expect(enqueued[0]).toMatchObject({ auditId: 'audit-1', target: '[redacted-domain]', phase: 'deep_scan' });
	});
	...
	it('runs runDeepScanFromStepStore and merges to csc_complement_full', async () => {
		...
		stored.set('a-1:[redacted-domain]:csc_complement_fast', {
			status: 'completed',
			payload: {
				viewVersion: 1,
				anchor: { apex: '[redacted-domain]', primaryRegistrar: { family: 'csc corporate domains', name: 'CSC', ianaId: null }, managedByCsc: true },
				registrarPortfolio: { totalApexes: 1, byFamily: [{ family: 'csc corporate domains', count: 1, percent: 100, exampleApexes: ['[redacted-domain]'] }], offPortfolioCount: 0, offPortfolioApexes: [] },
				shadowItHighlights: [],
		...
		await runDeepScanFromStepStore({ auditId: 'a-1', target: '[redacted-domain]', stepStore: stepStore as never, internalCall });

		const full = stored.get('a-1:[redacted-domain]:csc_complement_full') as {
			status: string;
		...
		expect(full.payload.postureSnapshot.apexes[0]).toMatchObject({ apex: '[redacted-domain]', grade: 'B+' });
	});
});
