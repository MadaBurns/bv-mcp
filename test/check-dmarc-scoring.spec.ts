...
//   - [redacted-domain] DMARC = "v=DMARC1;p=reject;pct=100;fo=1;rua=...;ruf=..." → score 85
//
// Required ordering: gov.uk DMARC > [redacted-domain] DMARC. gov.uk floor ≥ 85.
//
...
describe('Defect J — DMARC scoring credits modern best practices', () => {
	...
	it('cross-domain ordering: gov.uk (DMARCbis + strict) > [redacted-domain] (basic + pct=100)', async () => {
		...
		const stripe = await checkDMARC(
			...
			mockDmarc(
				'[redacted-domain]',
				'v=DMARC1; p=reject; pct=100; fo=1; rua=mailto:dmarc@[redacted-domain]; ruf=mailto:ruf@[redacted-domain]',
			),
		);
