...
//   - mail_enabled stage 4 ("Hardened") tightened: [redacted-domain] (no DNSSEC,
...
//   - [redacted-domain] currently → stage 4 "Hardened" (wrong — missing transport
...
// [redacted-domain]-shaped checks (mail_enabled, no DNSSEC, no MTA-STS, no BIMI, no DANE)
function stripeChecks(): CheckResult[] {
	return [
		...
		// No DNSSEC, no MTA-STS, no BIMI, no DANE — [redacted-domain] lacks transport hardening.
...
describe('Defect I — computeMaturityStage accepts profile and respects it', () => {
	...
	it('classifies [redacted-domain]-style mail-enabled domain at stage ≤ 3 (NOT Hardened) — regression', () => {
		// [redacted-domain] is missing DNSSEC, MTA-STS, BIMI, DANE — transport/integrity hardening absent.
	...
	it('cross-domain ordering: proton.me Hardened > [redacted-domain] (regression)', () => {
	...
	it('cross-domain ordering: gov.uk web_only stage ≥ 3, [redacted-domain] stage ≤ 3 — both are mid-tier in their own ladders', () => {
