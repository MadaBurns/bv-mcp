...
describe('queryPtrRecords', () => {
	it('queries reverse DNS and strips trailing dots', async () => {
		globalThis.fetch = vi.fn().mockResolvedValue(ptrResponse('192.0.2.1', ['mail.example.com']));

		const { queryPtrRecords } = await import('../src/lib/dns-records');
		const results = await queryPtrRecords('192.0.2.1');

		expect(results).toEqual(['mail.example.com']);
		expect(vi.mocked(globalThis.fetch)).toHaveBeenCalledWith(
			expect.stringContaining('1.2.0.192.in-addr.arpa'),
			expect.anything(),
		);
	});
	...
	it('returns empty array when no PTR records exist', async () => {
		const reverseName = '1.2.0.192.in-addr.arpa';
