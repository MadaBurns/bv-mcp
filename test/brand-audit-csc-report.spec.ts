...
describe('brand_audit_get_report with cscComplement', () => {
	it('attaches cscComplement from csc_complement_full when present', async () => {
		...
		await stepStore.put({ auditId: 'a-1', target: '[redacted-domain]', step: 'csc_complement_full', status: 'completed', payload: { viewVersion: 1, reportId: 'csc_rpt_full' } });
		await stepStore.put({ auditId: 'a-1', target: '[redacted-domain]', step: 'csc_complement_fast', status: 'completed', payload: { viewVersion: 1, reportId: 'csc_rpt_fast' } });
...
describe('brand_audit_status CSC stages', () => {
	it('reports fast_ready when csc_complement_fast is completed but full is absent', async () => {
		...
		const stepStore = createMemoryBrandAuditStepStore();
		await stepStore.put({ auditId: 'a-1', target: '[redacted-domain]', step: 'csc_complement_fast', status: 'completed', payload: {} });
	...
	it('reports deep_ready when csc_complement_full is completed', async () => {
		const { brandAuditStatus } = await import('../src/tools/brand-audit-status');
		const stepStore = createMemoryBrandAuditStepStore();
		await stepStore.put({ auditId: 'a-1', target: '[redacted-domain]', step: 'csc_complement_full', status: 'completed', payload: {} });
		await stepStore.put({ auditId: 'a-1', target: '[redacted-domain]', step: 'csc_complement_fast', status: 'completed', payload: {} });
