...
/**
 * Shared scoring-model test suite — the single source of truth for these tests.
 *
 * Run against BOTH import surfaces so source↔built (dist/DTS) drift is caught:
 *   - packages/dns-checks/src/__tests__/scoring/scoring-model.spec.ts → source (`../../scoring`)
 *   - test/scoring-model.spec.ts → built package (`@blackveil/dns-checks/scoring`)
 *
