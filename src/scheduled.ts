...
} from './lib/client-ip-audit';
import type { ClientIpAuditAssessment } from './lib/client-ip-audit';
...
/**
 * Suppression window for the `client_ip_header_missing` alert. The regression it
 * reports is a zone-config change that stays broken until an operator fixes it,
 * so the lane would otherwise re-page every 15 minutes for days. One page per
 * six hours (4/day) keeps it visible without flooding the channel.
 */
const CLIENT_IP_ALERT_COOLDOWN_SECONDS = 6 * 60 * 60;

/** KV marker (RATE_LIMIT namespace) written after a `client_ip_header_missing` dispatch. */
export const CLIENT_IP_ALERT_COOLDOWN_KEY = 'client-ip-audit:alerted';

/**
 * Public-door `cf-connecting-ip` presence lane (#896). Runs on the 15-min cron
 * beside `handleFuzzingScan`, dispatched by its own `ctx.waitUntil` in
 * `src/index.ts` so a failure here cannot mask the analytics lanes and vice versa.
 *
 * Why a separate handler and not a lane inside `handleScheduled`: that function
 * gates on `CF_ACCOUNT_ID` + `CF_ANALYTICS_TOKEN` because every lane there reads
 * Analytics Engine. This lane reads D1 (`mcp_access_log`, where the
 * `ip_masked = 'no-cf-header'` marker lives) and must keep running on a deploy
 * with no AE token at all.
 *
 * Cost: ONE aggregate SELECT (`clientIpHeaderAuditSql`) per tick, one KV read
 * only when degraded, one KV write only after a dispatch. Stays well inside the
 * cron's CPU budget and D1's per-invocation query cap.
 *
 * Verdicts (SSOT: `src/lib/client-ip-audit.ts`, shared with
 * `npm run audit:client-ip-headers`):
 *   - `degraded` (total >= 20 AND missing/total > 0.05) → one alert per
 *     CLIENT_IP_ALERT_COOLDOWN_SECONDS, aggregates only (never raw IPs, key
 *     hashes or session IDs);
 *   - `unknown`  (total < 20, or a malformed row) → logged, never alerted, and
 *     NEVER reported as healthy — "0 missing of 0" is absence of evidence;
 *   - `healthy`  → logged at info.
 *
 * Fail-soft throughout: a D1 error or KV blip is logged and the tick moves on.
 * The enforcement fix is the operator's zone-config change; this lane only
 * makes the regression visible.
 */
export async function handleClientIpHeaderAudit(env: ScheduledEnv): Promise<void> {
	if (!env.INTELLIGENCE_DB) return;
	// Same gate as every other alerting handler: no webhook → alerting is off.
	const webhookUrl = await resolveAlertWebhookUrl(env);
	if (!webhookUrl) return;
	...
	const logDetails = {
		message: 'client_ip_header_audit',
	...
	if (assessment.status !== 'degraded') {
		// `unknown` is logged as unknown — a quiet lane must be greppable, and an
		// empty window must never read as a pass.
		logEvent({
			timestamp: new Date().toISOString(),
			category: 'scheduled',
			result: 'client_ip_header_audit',
			severity: 'info',
			details: logDetails,
		});
		return;
	...
	// Per-lane cooldown, same mechanism as the fuzzing scan. Fail-soft on KV
	// errors: paging twice beats staying silent on a transient KV blip.
	let alreadyAlerted = false;
	if (env.RATE_LIMIT) {
		try {
			alreadyAlerted = (await env.RATE_LIMIT.get(CLIENT_IP_ALERT_COOLDOWN_KEY)) !== null;
		} catch {
			// KV down — proceed with alert (fail-loud rather than silent).
		}
	}
	...
	logEvent({
		...
		result: 'client_ip_header_audit',
		severity: 'warn',
		details: { ...logDetails, suppressedByCooldown: alreadyAlerted },
	});
	...
	await sendAlert(
		...
		buildAlertPayload({
			...
			// Above half the traffic every per-IP control (rate limits, quotas,
			// owner-tier gate) is effectively collapsed onto one 'unknown' bucket.
			severity: ratio !== null && ratio > 0.5 ? 'critical' : 'warning',
			metrics: {
				alert_kind: CLIENT_IP_HEADER_MISSING_ALERT_KIND,
				total_public_calls: total,
