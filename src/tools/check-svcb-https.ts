 ...
 *
 * HTTPS records (type 65) enable:
 * - Alt-svc style capability advertising (ALPN: h2, h3, etc.)
 * - Encrypted Client Hello (ECH) parameter distribution
 * - Hint-based connection optimization without an extra round-trip
 *
 ...
 * ALPN appears as `alpn="h2,h3"` or `alpn=h2,h3` in the data field.
 */
 ...
 * HTTPS records have format: `<priority> <target> [params]`
 * Priority 0 is an alias mode record; priority >= 1 is service mode.
 */
 ...
 * @returns CheckResult with SVCB/HTTPS findings
 */
export async function checkSvcbHttps(domain: string, dnsOptions?: QueryDnsOptions): Promise<CheckResult> {
	...
	try {
		httpsRecords = await queryDnsRecords(domain, 'HTTPS', dnsOptions);
	} catch {
		findings.push(
			createFinding(
				'svcb_https',
