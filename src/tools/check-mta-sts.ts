...
export async function checkMtaSts(domain: string): Promise<CheckResult> {
  ...
  try {
    ...
    const mtaStsRecords = txtRecords.filter((r) =>
      r.toLowerCase().startsWith("v=stsv1"),
    );
    ...
    findings.push(
      createFinding(
        "mta_sts",
        "MTA-STS DNS query failed",
        "low",
        `Could not query MTA-STS TXT record for ${domain}.`,
      ),
    );
  }
  ...
  try {
    ...
    const validRecords = tlsrptRecords.filter((r) =>
      r.toLowerCase().startsWith("v=tlsrptv1"),
    );
    if (validRecords.length === 0) {
      findings.push(
        createFinding(
          ...
          "No TLS-RPT record",
          "low",
          `No TLS-RPT record found at _smtp._tls.${domain}. TLS-RPT enables reporting of TLS connection failures.`,
        ),
      );
    }
  } catch {
