...
const EXPLANATIONS: Record<string, ExplanationEntry> = {
  SPF_PASS: {
    title: "SPF Validated",
    severity: "pass",
    explanation: "SPF (Sender Policy Framework) is properly configured. The domain specifies which mail servers are authorized to send email on its behalf.",
  ...
  SPF_FAIL: {
    ...
    recommendation: "Review your SPF record and ensure all legitimate email sources are included. Common issue: using -all but missing include statements.",
  ...
  DMARC_PASS: {
    ...
    severity: "pass",
    explanation: "DMARC (Domain-based Message Authentication, Reporting & Conformance) is properly configured with a policy that provides protection against email spoofing.",
    recommendation: "Monitor your DMARC reports to ensure legitimate email is not being blocked. Consider enabling reject policy for stronger protection.",
    references: ["https://datatracker.ietf.org/doc/html/rfc7489"],
  },
  DMARC_FAIL: {
    title: "No DMARC Record Found",
    severity: "fail",
    explanation: "DMARC builds on SPF and DKIM to provide email authentication policy. Without DMARC, receivers have no policy guidance for handling authentication failures.",
  ...
  DKIM_PASS: {
    ...
    severity: "pass",
    explanation: "DKIM (DomainKeys Identified Mail) is properly configured. Outgoing emails are digitally signed and can be verified by receivers.",
  ...
  DKIM_FAIL: {
    ...
    severity: "fail",
    explanation: "DKIM adds a digital signature to outgoing emails, allowing receivers to verify the email was sent by an authorized server and wasn't modified in transit.",
  ...
  DNSSEC_PASS: {
    ...
    severity: "pass",
    explanation: "DNSSEC is properly configured with valid cryptographic signatures. This protects against DNS spoofing and cache poisoning attacks.",
  ...
  MTA_STS_FAIL: {
    ...
    severity: "fail",
    explanation: "MTA-STS enforces TLS encryption for incoming email, preventing downgrade attacks where an attacker forces email to be sent unencrypted.",
...
export function formatExplanation(result: ExplanationResult): string {
  const lines = [
    `## ${result.title}`,
    `**Check Type:** ${result.checkType} | **Status:** ${result.status}`,
    "",
  ];
