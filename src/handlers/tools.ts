...
const TOOLS: McpTool[] = [
  {
    name: "check_spf",
    description: "Check SPF (Sender Policy Framework) records for a domain. Validates SPF TXT records for proper syntax, mechanisms, and policy.",
    inputSchema: DOMAIN_INPUT_SCHEMA,
  },
  {
    name: "check_dmarc",
    description: "Check DMARC (Domain-based Message Authentication) records for a domain. Validates _dmarc TXT records for policy configuration.",
    inputSchema: DOMAIN_INPUT_SCHEMA,
  },
  {
    name: "check_dkim",
  ...
  {
    name: "check_dnssec",
    description: "Check DNSSEC (DNS Security Extensions) status for a domain. Verifies if DNS responses are cryptographically signed and validated.",
    inputSchema: DOMAIN_INPUT_SCHEMA,
  },
  {
    name: "check_ssl",
  ...
  {
    name: "check_caa",
    description: "Check CAA (Certificate Authority Authorization) records for a domain. Validates which CAs are authorized to issue certificates.",
    inputSchema: DOMAIN_INPUT_SCHEMA,
  },
  {
    name: "scan_domain",
    description: "Run a comprehensive DNS security scan on a domain. Executes all checks (SPF, DMARC, DKIM, DNSSEC, SSL, MTA-STS, NS, CAA) in parallel and returns an overall security score and grade.",
    inputSchema: DOMAIN_INPUT_SCHEMA,
  },
  {
...
function formatCheckResult(result: CheckResult): string {
  ...
  if (result.findings.length > 0) {
    ...
    for (const finding of result.findings) {
      const icon = finding.severity === "info" ? "ℹ️" :
