...
//  * The Cloudflare `Env` types are generated HERMETICALLY by this script (see
//    `generateTestEnvTypes`, issue #1068) rather than inherited from the developer's own
//    `wrangler types` output, so a local `.dev.vars` cannot move the count.
//
// Usage
...
/** Cache directory for this gate's own generated inputs, repo-root-relative. */
const CACHE_DIR = 'node_modules/.cache/bv-mcp';

/**
 * Where the HERMETIC generated Cloudflare env types land, repo-root-relative. Referenced by
 * `test/tsconfig.json`'s `include`.
 *
 * Deliberately NOT the repo-root `worker-configuration.d.ts`: that copy is generated by
 * `npm run typecheck` FROM the developer's local `.dev.vars`, and this gate must not clobber
 * what their editor reads. Equally deliberately under `node_modules/.cache/` rather than a new
 * repo path — every ignore surface (git, eslint, prettier, gitleaks, the pre-commit generated-
 * file gate) already skips `node_modules/`, so a 13k-line generated file adds no lint noise and
 * needs no new ignore rule. `npm ci` may wipe it; this script regenerates it on every run.
 */
const TEST_ENV_TYPES = `${CACHE_DIR}/worker-configuration.d.ts`;

/**
 * Empty `.env` handed to `wrangler types --env-file`, repo-root-relative. It must EXIST —
 * wrangler exits non-zero on a missing `--env-file` path.
 */
const HERMETIC_ENV_FILE = `${CACHE_DIR}/hermetic.env`;
...
/**
 * Generate the Cloudflare `Env` types this gate typechecks against, HERMETICALLY (issue #1068).
 *
 * `wrangler types` folds the KEYS of a local `.dev.vars` into the generated `Env` as REQUIRED
 * `string` members. A developer checkout has that file; CI does not — so the identical source
 * tree read 522 errors locally and 520 in CI. The two extra were both
 * `{ ...env, OAUTH_SIGNING_SECRET: undefined } as TestEnv`: harmless while `Env` has no
 * `OAUTH_SIGNING_SECRET`, but once `.dev.vars` makes it a required `string` the assertion stops
 * overlapping and tsc raises TS2352. That machine-local +2 is not attributable to any branch's
 * diff, and it blocked three unrelated tickets whose verification re-runs this gate locally.
 *
 * Passing an empty `--env-file` suppresses local env-file discovery, so the generated `Env`
 * depends only on tracked `wrangler.jsonc` — measured identical (wrangler 4.131.1 content hash
 * `fd7eb8231cef6cb62b9dfa7c8e72f672`) with and without a `.dev.vars` present.
 *
 * Do NOT "fix" a future divergence here by banking it with `-- --update`: the baseline must not
 * encode a machine-local artifact.
 */
function generateTestEnvTypes() {
	const wrangler = path.join(REPO_ROOT, 'node_modules', 'wrangler', 'bin', 'wrangler.js');
	if (!existsSync(wrangler)) {
		fail(`wrangler is not installed at ${wrangler} — run \`npm ci\` first.`);
	}

	mkdirSync(path.join(REPO_ROOT, CACHE_DIR), { recursive: true });
	writeFileSync(path.join(REPO_ROOT, HERMETIC_ENV_FILE), '');

	const result = spawnSync(process.execPath, [wrangler, 'types', TEST_ENV_TYPES, '--env-file', HERMETIC_ENV_FILE], {
		cwd: REPO_ROOT,
		encoding: 'utf8',
	...
	const raw = `${result.stdout ?? ''}${result.stderr ?? ''}`;
	if (result.error) {
		fail(`failed to spawn wrangler types: ${result.error.message}`);
	}
	if (result.status !== 0) {
		fail(`wrangler types exited ${result.status}:\n${raw}`);
	}
	if (!existsSync(path.join(REPO_ROOT, TEST_ENV_TYPES))) {
		fail(`wrangler types reported success but ${TEST_ENV_TYPES} was not written.`);
	}

	// The generator stamps a content hash of the config it read into line 2. Printing it makes a
	// local-vs-CI comparison one glance instead of an artifact upload — exactly the divergence
	// that took #1068 four rounds to isolate.
	const [, hashLine = '', runtimeLine = ''] = readFileSync(path.join(REPO_ROOT, TEST_ENV_TYPES), 'utf8').split('\n');
	console.log(`typecheck:tests: generated ${TEST_ENV_TYPES} (hermetic — local env files excluded)`);
	console.log(`  ${hashLine.trim()}`);
	console.log(`  ${runtimeLine.trim()}`);
}
...
// `test/tsconfig.json` includes the generated env types. This gate generates them itself, from
// tracked config only, so the run cannot inherit whatever a developer last produced.
generateTestEnvTypes();
