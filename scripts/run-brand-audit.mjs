...
const domains = ['amazon.com', 'apple.com', 'google.com', 'microsoft.com', 'disney.com', 'nike.com', 'paypal.com', 'stripe.com', 'walmart.com', 'github.com'];

const CSC_GLOBAL = 'csc corporate domains';
...
async function auditDomain(target) {
    ...
    try {
        ...
        const consolidated = rdapResults.filter(r => r.registrar.toLowerCase().includes('csc corporate domains'));
        const shadowIt = rdapResults.filter(r => !r.registrar.toLowerCase().includes('csc corporate domains') && r.confidence >= 0.7);
        const impersonation = rdapResults.filter(r => !r.registrar.toLowerCase().includes('csc corporate domains') && r.confidence < 0.7);
