...
- `check_authoritative_dns_infra` finding **“Authoritative DNS infra probe not configured”** or `check_root_server_set` finding **“Official root hints embedded”** with `partial: true`: `BV_INFRA_PROBE` is not bound in this deployment. Worker-only mode returns structured partial evidence and marks raw UDP/TCP DNS, BGP, RPKI, and vantage checks inconclusive. Provision the infra probe worker and `BV_INFRA_PROBE` service binding to run the live evidence checks.
- `check_authoritative_dns_infra` or `check_root_server_set` finding **“… checks inconclusive”** with `metadata.unprovisioned: true` and `probeErrors` naming `live_raw_dns_probe_not_configured` / `live_root_server_set_probe_not_configured` (`checkStatus: 'error'`, `score: 0`, `partial: true`): `BV_INFRA_PROBE` is bound and answering, but the probe worker shipped in this repo (`src/workers/infra-probe.ts`) does not yet issue live raw-DNS or root-zone queries — those lanes return embedded root hints as reference data only. Nothing was measured, so no verdict is reported and the category is excluded from scoring. This is a provisioning state, not a transient failure: retrying or `force_refresh` returns the same result.
