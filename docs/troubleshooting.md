...
- `check_http_security` finding **“HTTP security check timed out”** (`checkStatus: 'timeout'`, `missingControl: true`): the total 10s budget (dual-fetch + WAF body + package probe) was exhausted. The host was unreachable or extremely slow. The scan still completes — the `checkStatus: 'timeout'` marker excludes the category from the score as inconclusive (remaining categories renormalised) rather than blocking other checks.
