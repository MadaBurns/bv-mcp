...
deploy — no window with alerting silently off, and no override needed:
...
3. Deploy with it:
       ...
       npm run deploy:prod -- --secrets-file .dev/alert-webhook.secrets.json
...
4. The next `npm run deploy:prod` (no override needed) runs the gate normally
   and confirms the secret is live.
