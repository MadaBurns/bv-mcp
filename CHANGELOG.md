...
- **An unfingerprinted UA/TLS-based block on the `ssl` check no longer publishes a false "No HSTS header" or "No HTTP to HTTPS redirect" (#972).** A non-Cloudflare origin that answers the scanner's probe with a 401/403/429/202 block (no vendor body signature to fingerprint) previously fell through to the "real page" branch on both the `https://` (HSTS) and `http://` (redirect) legs, so the block page's empty header set and non-redirect status were read as the site's own answer — reported live as six false "header missing" findings, one a confident `high`, on an origin whose real headers (HSTS at `preload` included) are all present. `check-http-security`'s own 401/403/"other 4xx" abstention (issue #638) already covered the header-security half of this; `check-ssl`/`ssl-analysis` had no equivalent guard for either leg. Both legs now route 401/403/429/202 (plus the pre-existing 5xx/no-content lanes) to the same unmeasured/`checkStatus: 'error'` abstention, never `missingControl` — a blocked probe never reached the origin, so it must not be scored as absence.
