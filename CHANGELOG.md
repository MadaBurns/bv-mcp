...
_Historical entries below were edited on 2026-09-09 to remove a third-party vendor name; semantics unchanged._
...
- Registrar-complement builder module (`src/lib/`) — fast-stage payload builder.
- Registrar-complement deep-scan module + its job runner (`src/lib/`) — queue-backed per-apex `scan_domain` + `discover_subdomains` orchestration; phase='deep_scan' messages on BRAND_AUDIT_QUEUE.
- Registrar-complement Zod schema (`src/schemas/`, v1).
