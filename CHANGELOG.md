...
- **A partial-outage scan no longer turns transient check failures into remediation actions or priced upsells.** The 3.37.0 evidence-gate work made `generate_fix_plan` and `map_csc_products` abstain when NO check completed, but on a scan with ≥1 completed check (so the gate correctly doesn't fire) findings from transient checks (`checkStatus: 'timeout' | 'error'` — the `buildDnsErrorResult`/`safeCheck` shape) still flowed into confident outputs: `generate_fix_plan` rendered up to 14 bogus "Fix X: X check error" actions on a 5-of-19 scan, and `map_csc_products`' `evalScanProduct` priced a transient DMARC/SSL/DNSSEC category as a `recommended: true, priority: 'high'` upsell — `assessed: true`, no caveat — off a "check error" finding that measured nothing. Both now apply the same per-category transient exclusion `map_compliance` already applied per control (closing the cross-tool asymmetry): `generate_fix_plan` takes actionable findings from COMPLETED checks only, and `map_csc_products` gives a transient category a per-product abstention (`recommended: false, priority: 'none'`, a new additive `notAssessed: true` wire field, and a "not assessed — check attempted, did not complete (transient DNS/network failure)" justification) rendered as `NOT ASSESSED`/`?` — never as "OK", never as the "not observed" sellable-gap wording, and never as a priced gap. Real findings from completed checks — including the genuine "category absent → not observed" lead when the scan has completed evidence — are untouched: an incomplete measurement must never produce a confident output, and a real measurement must never be suppressed. The shared single-check predicate is the new `isCompletedCheck` in `src/lib/ungraded-display.ts` (`hasCompletedEvidence` is now defined over it).

### Added

- **Two tripwire audits.** `test/audits/completed-predicate-agreement.audit.test.ts` iterates every `CheckStatus` union member (parsed from the type declaration itself, so a future fourth member joins automatically) through the three deliberate spellings of the completed-check predicate — `computeScanEvidence` in `packages/dns-checks/src/scoring/evidence.ts`, `isCompletedCheck`/`hasCompletedEvidence` in `src/lib/ungraded-display.ts`, and `map_compliance`'s negative `!== 'timeout' && !== 'error'` spelling at both its per-control filter and report-level `assessed` — and fails on the first disagreement (mutation-verified: adding a hypothetical `'skipped'` member fails it). `test/audits/scanscore-fixture-evidence.audit.test.ts` closes the gap that `test/` is excluded from `tsconfig.json`, so a hand-built `{ ... } as ScanScore` fixture silently drops the REQUIRED `evidence` field the cast cannot enforce: every plain object-literal cast in both un-typechecked test trees must carry `evidence:` (or a spread), with `as unknown as ScanScore` as the documented escape hatch for deliberately malformed fixtures (also mutation-verified). whenever crt.sh was unreachable.** The direct (non-`BV_CERTSTREAM`) enumeration path depended on a single public CT source: any crt.sh timeout, 5xx, or network error returned `sourceUnavailable` with zero subdomains, so for a domain with many live subdomains the tool reported nothing for hours at a time (observed against `bnz.co.nz`, 2026-07-26). The Certspotter endpoint referenced in the shared certstream layer was never reachable from this path, so there was no failover in practice. The path is now resilient in three layers: (1) **real multi-source failover** — crt.sh → Certspotter, each normalized into the same aggregation and tried in order until one answers authoritatively, with a bounded same-source retry plus backoff on a transient outcome; (2) **last-known-good fallback** — a clean enumeration is cached per domain (7-day TTL, `SCAN_CACHE` KV) and re-served on a total live-source outage marked `stale: true` with `cacheAgeMinutes` and an explicit staleness banner in the formatted output, because a flagged stale list is far more useful to a security caller than an empty result; (3) **per-source observability** — every attempt emits a `ct_source` log line carrying the source and its outcome (`ok`/`empty`/`http_error`/`timeout`/`error`) so upstream outages are visible and alertable instead of silently degrading the tool. The stale fallback is also consulted when the 24s synchronous budget trips before a direct source can run (a KV read costs milliseconds, unlike the 10s fetch there is no longer budget for), so a slow certstream fast path degrades to data rather than an error. The explicit `sourceUnavailable` error (`isError: true`) is now reached only when every live source failed *and* no cache exists; a genuine "queried, none found" answer is still distinct from both. `force_refresh` continues to bypass the certstream fast-path cache but no longer suppresses the stale fallback — a total outage must never return a passing empty result. The last-known-good cache is never written from a `sourceUnavailable`, `partial`, or already-`stale` result, so an outage cannot poison the fallback.
