...
- **A deployed-but-broken MTA-STS policy no longer scores worse than having none.** Measured: no policy 85/pass, `mode: none` 85/pass, MX not covered 75 — but a policy omitting `max_age` scored **0/failed**. A domain that deployed the control and got one field wrong was ranked below one that never tried. Now 88/pass, giving the ladder `enforce 100 > testing 95 > MX-gap 90 > RFC-invalid 88 > mode:none 85 = absent 85`. `mode: none` deliberately stays level with silence: RFC 8461 §5 designates it the sanctioned withdrawal path, so scoring it below absence would penalise the prescribed removal procedure. No parity fixture expectation moved.
