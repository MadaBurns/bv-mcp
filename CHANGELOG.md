...
⚠️ **`SCORING_MODEL_VERSION` is still `1.10.0` in this diff and MUST be bumped to `1.11.0` before this block is released.** Its own doctrine requires a bump for any change that alters scores or grades — profile weights and severity penalties are named in that list, and this release moves both. The edit is an outstanding operator action; the block is not releasable until it lands. This block's header opened "**No scoring-model change.** `SCORING_MODEL_VERSION` stays 1.10.0" while every entry in it was score-neutral; the escalation makes that false, so the header is corrected above rather than left to contradict the entry beneath it.
