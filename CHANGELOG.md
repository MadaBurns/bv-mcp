...
- `SCORING_MODEL_VERSION` → `1.4.0`, because profile *detection* changed (see the `failureRatio` correction above). A report consumer can use this stamp to detect that two scans of the same domain ran under different scoring policies.
- **Grade cut-points, grade bands, category weights, profile weights, tier split, severity penalties and the check matrix are all unchanged.** Both grade scales are unchanged: the canonical 9-band `scoreToGrade` and the NIST 6-band `nistScoreToGrade` at the `displayGradeFor` chokepoint. This change alters only *whether* a grade is emitted or can be absent, never what an emitted grade means. A domain whose checks all completed scores exactly what it scored before — measured and reported at implementation time.
