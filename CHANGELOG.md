...
- **Cloudflare test-toolchain bump clears 5 high-severity dev-dependency advisories.** `ci-contract.yml`'s `npm audit --audit-level=high` gate (which, unlike `security.yml`'s `--omit=dev` gate, includes devDependencies) went red on newly published advisories affecting the Workers test runtime: `brace-expansion <=5.0.7` (DoS via unbounded expansion, GHSA-mh99-v99m-4gvg) reached through `eslint` → `minimatch`, and `sharp <0.35.0` (inherited libvips CVE-2026-33327 / -33328 / -35590 / -35591) reached through `miniflare` → `wrangler` / `@cloudflare/vitest-pool-workers`. Bumps `wrangler` `^4.112.0` → `^4.114.0`, `@cloudflare/vitest-pool-workers` `^0.18.6` → `^0.18.8`, and `@cloudflare/workers-types` `5.20260721.1` → `5.20260724.1`; the `workers-types` pin is required, not incidental — `wrangler@4.114.0` declares `peerOptional @cloudflare/workers-types@^5.20260722.1`, and the old exact pin was the ERESOLVE conflict that made `npm audit fix` unrunnable. Transitively lands `miniflare@4.20260722.0`, `sharp@0.35.2`, `brace-expansion@5.0.8`. Runtime dependencies are untouched; no production code changed.
