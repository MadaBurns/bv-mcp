...
- **Worker validation can no longer report success while hiding runtime failures.** The global Vitest unhandled-error suppression is removed, unread HTTP response bodies are cancelled safely without floating rejections, Node-only report tests are isolated from the Workers pool, and typed `no-floating-promises` linting now covers Worker source.
