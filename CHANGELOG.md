...
- **The same false attribution on every other uniform-set nameserver platform.** #929's fix listed only one.com; any platform that hands every tenant the same NS set but was not in `SHARED_NS_APEXES` still reached the "dedicated" `ns_set_match` arm and reported `owned_by_seed` / `strong` on a complete match. Measured across 14,062 Tranco domains (two DoH vantages, 2026-09-09), the list now carries every platform on which an identical complete set was observed on unrelated tenants: Hostinger, Wix, Squarespace Domains (+ NS1 pools), Google Cloud DNS (five fixed sets — was pinned as unlisted), IONOS, Bluehost, Strato, Hetzner (Console and Robot), OVH, `digital.govt.nz` (13 NZ agencies on one set), DigitalOcean, Linode, Vercel, Hover, DNSimple, DreamHost, SiteGround, Porkbun, EuroDNS, Dynadot, a corporate brand-protection registrar (`cscdns.net`), MarkMonitor, DNS Made Easy, Constellix, UltraDNS, Azure DNS, Network Solutions, ClouDNS, Namecheap hosting/Spaceship, and the regional defaults of Alibaba, HiChina, DNSPod, dnsv4, share-dns, Xinnet, REG.RU, Timeweb, Beget, Selectel, RU-CENTER, Yandex, GMO, Sakura, ns14.net and Aruba. None is pooled; a same-platform candidate is `unattributed` (identical sets) or `third_party` (platform hosts plus its own), never `owned_by_seed`. Cloudflare and Route 53 stay ownership-bearing on the same measurement. Mechanism unchanged; not `scan_domain`-score-affecting. (Refs #939)
  - **Behaviour change on enterprise-tier output, disclosed rather than framed as pure fix.** a corporate brand-protection registrar (`cscdns.net`/`.uk`), MarkMonitor, `digital.govt.nz` and UltraDNS are not self-service, so a seed's OWN same-platform defensive registration drops from `owned_by_seed` / `strong` to `unattributed` as well (measured live: `natwest.com` → `natwest.co.uk`, both on the enterprise-gated registrar's `udns1`/`udns2` pair). Two customer-visible effects: `check_lookalikes` now emits the uncapped threat observation for such a name (`medium` with MX; `high` with MX plus a recent-registration / disposable-MX / dark-web corroborator) instead of a single `info` finding, and `check_shadow_domains` clamps it to `info`, so a customer's own variant on that registrar's platform with no DMARC loses the `high`/`medium` "lacks DMARC/SPF" finding it received while wrongly attributed (it is still reported when that domain is scanned directly). Accepted under #937's rule — a verdict may rest only on what the seed alone publishes; the own-name wording is tracked in #949. The other ~83 listed apexes are self-service (a squatter can land on the seed's exact set for the price of an account) and must be listed regardless.
