...
- **The MCP `initialize` handshake described a BUSL-1.1 product as "Open-source".** `serverInfo` is the first string every AI client reads, so an agent quoting it repeated the claim; the brand-voice rule is *"say 'source-available', never 'open-source'; that licence is not open-source."* Fixed at the handshake and on the public landing page, with a repo-wide audit sweeping every tracked file so the phrase cannot reappear in a doc, tool description or manifest. (#986)
