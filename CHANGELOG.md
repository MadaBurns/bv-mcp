...
- **A deployed-but-broken MTA-STS policy no longer scores worse than having none.** Measured: no policy 85/pass, `mode: none` 85/pass, MX not covered 75 — but a policy omitting `max_age` scored **0/failed**. A domain that deployed the control and got one field wrong was ranked below one that never tried. The ladder is now `enforce 100 > testing 95 > MX-gap 90 > RFC-invalid 88 > unretrievable 85 = mode:none 85 = absent 85`. `mode: none` deliberately stays level with silence: RFC 8461 §5 designates it the sanctioned withdrawal path, so scoring it below absence would penalise the prescribed removal procedure. No parity fixture expectation moved.

  ⚠️ **The first cut of this ladder over-corrected, and its own tests froze the mistake.** Every policy defect was put at 88 — including a policy that cannot be retrieved at all. Publishing `_mta-sts.x TXT "v=STSv1; id=1"` plus a CNAME to any host that 404s needs no HTTPS service, no certificate and no policy file, yet banked **+3 over a domain that deployed nothing**. Two DNS records bought the points. Unretrievable branches (404/5xx, redirect away from the well-known URL, body over the RFC 8461 64 KB ceiling) now carry a separate `MTA_STS_POLICY_UNRETRIEVABLE_PENALTY = 15` and **tie** absence; 88 is reserved for a policy that really is being served and is merely RFC-invalid. The line is RETRIEVABILITY, not validity — a served policy file is a real partial deployment and gets credit for the part that exists. The ladder tests now assert the INVARIANT ("no unretrievable branch outscores absence") rather than the literal, because asserting the literal is exactly how 88 got pinned.

  ℹ️ **On the `high` → `medium` severity downgrade of those four findings — deliberate, and deliberately not reverted.** A security review correctly observed that severity is read by three tools outside the scoring path (`validate-fix.ts:60` `BLOCKING_SEVERITIES`, `compare-baseline.ts:270` `max_high_findings`, `analyze-drift.ts:110` `classifyDrift`), so the downgrade makes a broken MTA-STS policy stop blocking a customer's CI gate, and recommended restoring `high`. That restores a worse defect. The absence finding (`check-mta-sts.ts:221`) was **already** `medium` before this release and is untouched by it — so the pre-existing state blocked a domain that half-deployed the control while waving through one that deployed nothing at all. That is the same "punish the tryer" asymmetry this entry fixes in the score dimension, sitting unnoticed in the severity dimension. Aligning broken with absent is the correction, not the regression; the 85-vs-100 score carries the signal, and severity and penalty are independent now that `penaltyOverride` exists.
