...
- **`map_csc_products` no longer sells three products for a domain nobody measured.** The producer set `assessed` and the formatter never read it, so an unmeasured domain rendered `**Score:** not measured | **3** recommended` followed by three priority-tagged upsells justified by "DMARC not observed" — recommendations derived entirely from non-observation. `prioritize_csc_leads` already suppressed exactly this from the same producer output; the two tools gave opposite answers about one input. The three scan-driven products now abstain at the producer (`recommended: false, priority: 'none'`) when no check ran, and the formatter prints the shared note (`No checks ran for this domain, so no product gap could be assessed.`) with no product list and no recommended-count claim. A MultiLock recommendation resting on RDAP evidence — fetched independently of the scan — is deliberately still reported.
...
- **`batch_scan`'s per-domain prose distinguishes "evidence insufficient" from "not measured".** A domain whose scan attempted checks but fell under the evidence-sufficiency threshold now renders `evidence insufficient (N/M checks completed)`; a domain that was never measured at all (invalid input, budget exhaustion, a throw before any check ran) keeps the plain `not measured` line — the two previously collapsed into the same wording. `CscProductReport` gains a required `caveat: string | null` + `caveatKind: 'never_ran' | 'all_transient' | null` pair; `CscLead` gains the same `caveat` / `caveatKind` pair threaded straight from the producer. Both are additive on the wire.
...
- **`bv-web-prod` must re-vendor the `@blackveil/dns-checks` tarball at 1.7.0** and absorb, beyond the nullable `ScanScore.overall` / `.grade` already tracked above: (a) the required `ScanScore.evidence` field (optional `evidenceInsufficient` / `evidenceNote`) and (b) the `inconclusiveCategories` / `notApplicableCategories` disjointness — a consumer reading `notApplicableCategories` as the union of "not scored" must switch to `notApplicableCategories.concat(inconclusiveCategories)`. Its `parity.contract.test.ts` will fail on drift until it does. Approved by the operator ahead of this change; tracked as a follow-up issue on that repo — it cannot be verified from this repo. Consolidated compatibility note for that re-vendor, covering every wire-shape change in this release:
  ...
  **Shape narrowed for a state that used to over-report:**
  ...
  - `map_csc_products`' `CscProductReport.recommendations` still always carries the four products in fixed order, but when `assessed` is `false` the three scan-driven entries are now `recommended: false, priority: 'none'` with a `"… not assessed — no checks ran"` gap (they were `recommended: true, priority: 'low'`, derived from non-observation), so `recommendedCount` is `0` rather than `3` for such a domain. A consumer counting `recommended` entries as a sales surface will see those domains drop out — which is the intent. `csc_multilock` is unaffected: it rests on RDAP evidence fetched independently of the scan.
  ...
  **New members / fields (additive):**
  ...
  - `CscProductReport` gains `caveat: string | null` and `caveatKind: 'never_ran' | 'all_transient' | null`. `CscLead` gains the same `caveat` / `caveatKind` pair, threaded straight from the producer.
