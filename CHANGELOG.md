...
1. **PII / dead-key scrub**: replaced `csc-global` → `redacted-tenant`, `harlan.blackveilsecurity.com` → `internal-doh.example.com`, and 3 dead BV API keys (already revoked in prod) → `*_REDACTED_*`. Removed `.playwright-mcp/` browser-test snapshots and 2 internal docs (`docs/enterprise-architecture.md`, `docs/smithery-docs-nav.md`).
2. **OSS hygiene**: removed 6 internal-only docs from all history (`docs/architecture.md`, `docs/AS-BUILT.md`, `docs/phase8-golive.md`, `OAUTH_LOCAL_DEBUG.md`, `MCP.md`, `GEMINI.md`). Replaced one stale ref in `.github/instructions/scan-orchestration.instructions.md`. Added drift catchers to `.gitignore` (`/BENCHMARK_*.md`, `/CAPACITY_*.md`, `/CSC-*.md`, `/*-Call-Prep.md`, `/AS-BUILT.md`, `/phase*-golive.md`, `/MCP.md`, `/GEMINI.md`).
