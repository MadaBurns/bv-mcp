...
- **Vendor-neutral names for the registrar product-gap tooling and the brand-audit registrar view.** Two tools are renamed with no alias — `prioritize_csc_leads` → `prioritize_portfolio_leads` and `map_csc_products` → `map_registrar_products` (the latter stays internal-only). In their structured output the product key `csc_multilock` is now `registry_lock` (display name "Registry lock") and the per-lead field `recommendedCscProducts` is `recommendedProducts`; prose headings no longer name a vendor. The `brand_audit_*` view `csc_complement` is now `registrar_complement` (schema, queue message, tool descriptions and the enterprise-tier error string); its payload is wire version **2** — `anchor.managedByCsc` → `anchor.managedByRegistrar`, the registrar family key `csc corporate domains` → `corporate domains registrar`, and new report identifiers carry the `reg_rpt_` prefix. Report identifiers issued under earlier prefixes still validate (the `reportId` pattern is prefix-agnostic). Brand audits still in flight at deploy time under the old step-store keys will not resume their registrar view; start them again. `scripts/promote-brand-report.mjs` now writes to the ignored `.client-reports/` directory. The repo-safety scanner's client-context phrase gate moved to the same hashed list mechanism the client-domain gate uses, so the guard no longer carries the phrase it blocks.
- **MIGRATION:** the two old tool names above are no longer registered (no alias); calls to them return the standard unknown-tool error. Update clients to the new names.
