...
- **Profile detection now requires positive evidence of a control's presence.** `detectDomainContext` credited a control as "present" from a bare `passed === true` — which is also true for a check that observed *no record* (nothing to fail). A domain with sparse/absent records could therefore masquerade as a hardened `web_only` profile or over-fire into `enterprise_mail`. Detection now requires an actually-observed record (a non-absence, non-`missingControl` finding) before crediting SSL/CAA/MTA-STS/BIMI/DKIM as present. Logic-only — profile weight tables are unchanged. Note: detection is score-bearing on the `auto` path, so previously-misclassified sparse domains shift toward the correct (less lenient) profile.
