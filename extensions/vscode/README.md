...
**76 DNS & email security tools** for GitHub Copilot Chat. No install, no API key — just ask Copilot to scan any domain.
...
That's it. All 76 tools are available instantly.
...
The extension exposes the hosted MCP `tools/list` surface: 76 tools, including 35 `check_*` tools. The table below groups the most common workflows; Copilot can call every registered tool returned by the server.
