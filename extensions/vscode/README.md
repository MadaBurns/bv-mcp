...
**72 DNS & email security tools** for GitHub Copilot Chat. No install, no API key — just ask Copilot to scan any domain.
...
That's it. All 72 tools are available instantly.
...
The extension exposes the hosted MCP `tools/list` surface: 72 tools, including 33 `check_*` tools. The table below groups the most common workflows; Copilot can call every registered tool returned by the server.
